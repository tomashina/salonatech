...
public function save($file, int $quality = 90) {
	...
	if (is_object($this->image) || is_resource($this->image)) {
		...
		if ($extension == 'jpeg' || $extension == 'jpg') {
			...
			// Flatten transparency on white background for JPG
			$flatten = imagecreatetruecolor($this->width, $this->height);

			$white = imagecolorallocate($flatten, 255, 255, 255);
			imagefilledrectangle($flatten, 0, 0, $this->width, $this->height, $white);

			imagealphablending($flatten, true);
			imagecopy($flatten, $this->image, 0, 0, 0, 0, $this->width, $this->height);

			imagejpeg($flatten, $file, $quality);
			imagedestroy($flatten);
