...
class ModelExtensionShippingWeight extends Model {
    ...
    // Ako je ZIP 21405 – radimo posebnu logiku
    if (!empty($address['postcode']) && $address['postcode'] == '21405') {

        // Obradi samo GEO zonu 9
        if ($result['geo_zone_id'] != 9) {
            continue;
        }

        // Provjeri da je metoda za geo zonu 9 uključena
        if ($this->config->get('shipping_weight_9_status')) {
            $status = true;   // FORSIRAMO da vrijedi za taj ZIP
        } else {
            $status = false;
        }
        ...
        // ORIGINALNA LOGIKA za sve ostale ZIP-ove
